...
This file contains code to plot the exponents of fitness relaxation as a function of \mu.
"""
...
def make_exponent_plot(data_folders: list, 
                       nexps: int, 
                       L: int,
                       inner_folder: str, 
                       bac_str: str,
    ...
    """ Fit exponents to fitness relaxation as a function of \mu. """
    ...
    for ii, vary_val_folder in enumerate(data_folders):
        ...
        for curr_estimate in range(n_bootstraps):
            ...
            p0 = [1e-5, b_estimate, 2.0]
            lbs = [0, .99*b_estimate, 1e-5]
            ubs = [np.inf, 1.01*b_estimate, np.inf]
            ...
            print("Finished fitting on bootstrap %d/%d for %s=%e, \
                  ...
                  (curr_estimate+1, n_bootstraps, vary_str, vary_vals[ii], 
                      ii+1, len(data_folders), asymp_pow_params[-1]))
    ...
    print('mean:')
    print(np.mean(exponents, axis=0))
    print(np.mean(exponents, axis=0) - sem(exponents, axis=0))
    print(np.mean(exponents, axis=0) + sem(exponents, axis=0))
...
if __name__ == '__main__':
    ...
    make_exponent_plot(data_folders, nexps, L, inner_folder,
