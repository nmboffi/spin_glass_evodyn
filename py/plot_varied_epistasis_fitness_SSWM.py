...
This file contains code to visualize fitness trajectories as a function
of the epistasis parameter \beta in the SSWM regime.
"""
...
def make_epistasis_plot(data_folders: list,
    ...
    fig, ax = plt.subplots()
    if not log_plot:
        axins = ax.inset_axes([0.20, 0.375, .45, .4])
    ...
    if log_plot:
        ...
        axins.set_ylim([1.9, 2.035])
        axins.set_xlim([2.0e7, 1.2e8])
