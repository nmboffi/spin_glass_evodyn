...
def plot_increment(times: list,
    ...
    for ii, beta_val in enumerate(beta_vals):
        ...
        plt.plot(times[ii][inds], replicate_mean_incs[ii][inds],
                 color=cmap_fits[ii+2], label=r"$\beta=%0.2f$" % beta_val,
                 lw=2.5)
        ...
        if xlabel == 'days':
            ...
            # avoid plotting the single remaining mutation in the
            # \beta = 0 simulation.
            if beta_val > 0:
                axins.plot(times[ii][inds], replicate_mean_incs[ii][inds],
                           color=cmap_fits[ii+2], lw=1.25)
                axins.fill_between(times[ii][inds],
                                   replicate_mean_incs[ii][inds] + replicate_sem_incs[ii][inds],
                                   replicate_mean_incs[ii][inds] - replicate_sem_incs[ii][inds],
                                   alpha=0.3, color=cmap_fits[ii+2])
    ...
    if xlabel == 'days':
        ax.set_xscale('log')
        axins.set_xlim([5e4, 1e6])
        axins.set_ylim([1.75e-4, 1e-3])
...
def plot_rank(times: list,
    ...
    for ii, beta_val in enumerate(beta_vals):
        ...
        if xlabel == 'days':
            if ii == 0:
                axins = ax.inset_axes([0.525, 0.55, .425, .35])
            axins.plot(times[ii], replicate_mean_ranks[ii], color=cmap_fits[ii+2],
                       label=r"$\beta=%0.2f$" % beta_val, lw=1.25)
    ...
    if xlabel == 'fitness':
        ...
        plt.legend(ncol=1, loc='lower left', framealpha=0.5)
...
def plot_effective(times: list,
    ...
    for ii, beta_val in enumerate(beta_vals):
        ...
        effective = effectives[ii, :][inds]
        plt.plot(times[ii][inds], effective/effective[0],
                 color=cmap_fits[ii+2], label=r"$\beta=%0.2f$" % beta_val, lw=2.5)

        # make fitness and approximate fitness plot
        interp_effective = interp1d(times[ii][inds], effective, kind='cubic')

        # right-hand side of the effective fitness dynamics
        def rhs(y, t):
            # linear extrapolation in case the ODE solver goes outside the range
            if t > times[ii][inds][-1]:
                return effective[-1] + \
                    (effective[-1] - effective[-2])*(t - times[ii][inds][-1]) \
                    / (times[ii][inds][-1] - times[ii][inds][-2])
            else:
    ...
    for ii, beta_val in enumerate(beta_vals):
        plt.plot(times[ii], fits[ii], color=cmap_fits[ii+2],
                    label=r"$\beta=%0.2f$" % beta_val, lw=2.5)
...
def plot_delta_rank(fits: list,
    ...
    plt.plot(beta_vals, dRs[:, 1], color=cmap_fits[ii+2], lw=3)
    ax.fill_between(beta_vals, dRs[:, 0], dRs[:, 2], color=cmap_fits[ii+2], alpha=0.35)
    ...
    if savefig:
       plt.savefig("%s/rank_decrease.pdf" % output_folder,
                   dpi=300, transparent=True, bbox_inches='tight')
       np.save("data_proc/paper_figures/paper_figure_data/clonal_expected_rank.npy", dRs)
...
if __name__ == '__main__':
    ...
    beta_vals      = np.sort(
        np.array([float(folder.split('L')[-1].split('b')[-1]) \
    ...
    cmap_fits = cubehelix_palette(n_colors=len(beta_vals)+2, start=.5,
                                  rot=-.75, light=.85, dark=.1, hue=1,
                                  gamma=.95)

    make_epistasis_plot(data_folders, nexps, inner_folder, bac_str, mut_str,
                        beta_vals, L, base_folder, start_index, plot_skip,
