...
def make_epistasis_plot(
    ...
    if vary_str == 'mu':
        ...
        if vary_scale:
            ...
            for vary_ind, _ in enumerate(vary_vals):
                tc_ind = np.argmax(replicate_mean_fits_array[vary_ind, :] > Fc)
                dF = replicate_mean_fits_array[vary_ind, tc_ind] - 1.0
                dt = times[vary_ind, tc_ind] - times[vary_ind, 0]
                init_deriv = dF/dt
                times[vary_ind, :] *= init_deriv

            rescaled_derivs = (replicate_mean_fits_array[:, 1] - replicate_mean_fits_array[:, 0]) \
                    / (times[:, 1] - times[:, 0])
            print(f'rescaled derivatives: {rescaled_derivs}')
    ...
    for ii, vary_val in enumerate(vary_vals):
        ...
        if log_diff_plot:
            ...
            print(max_val, np.max(plot_means))
    ...
    plt.legend(ncol=1, framealpha=1.0, loc='best')
    plt.xlabel(x_label)
    plt.ylabel('fitness')
...
if __name__ == '__main__':
    ...
    bac_str        = 'bac_data*'
    skip           = 1
    # base_folder    = '/home2/nick/research/lenski/mu_scan_3_25_22_epi_same_landscape_noreset_fixrank'
