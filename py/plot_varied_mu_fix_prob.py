    ...
    for mu_index, mu_val in enumerate(mu_vals):
        ...
        if plot_analytical_curves:
            ...
            compute_bin_averaged_replicate_means(replicate_mean_fix_count,
                                                 ...
                                                 fix_count_data)
            plot_indices = replicate_mean_fix_count > 0.15*np.max(replicate_mean_fix_count)
        else:
...
if __name__ == '__main__':
    ...
    # file_str = 'fix_prob_vary_mu_windows_count_onwards.pickle'
    # file_str = 'fix_prob_vary_mu_full.pickle'
    file_str = 'fix_prob_vary_mu_full_prediction.pickle'
    n_time_points = 15
    file_indices = np.arange(5001)
    # bin_indices = np.arange(1, 3)
