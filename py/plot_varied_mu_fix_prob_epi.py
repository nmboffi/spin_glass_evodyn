...
def construct_individual_figure(xlabel: str,
    ...
    main_curve_alpha = 1.0 if not plot_analytical_curves else 0.6
    ...
    for mu_index, mu_val in enumerate(mu_vals):
        ...
        if plot_analytical_curves:
            ...
            compute_bin_averaged_replicate_means(replicate_mean_fix_count,
                                                 ...
                                                 fix_count_data)
            plot_indices = replicate_mean_fix_count > 0.20*np.max(replicate_mean_fix_count)
        else:
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    # plt.legend(ncol=2, loc='best', framealpha=0.75)
    plt.tight_layout()
